...
exports.drop = function (driver, opts, cb) {
	...
	queries.push("DROP TABLE IF EXISTS " + driver.config.schema + '.' + opts.table);

	for (i = 0; i < opts.many_associations.length; i++) {
		queries.push("DROP TABLE IF EXISTS " + driver.config.schema + '.' + opts.many_associations[i].mergeTable);
	}
...
exports.sync = function (driver, opts, cb) {
	...
	var primary_keys = opts.id.map(function (k) { return k; });
	var keys = [];
	...
	for (k in opts.allProperties) {
		prop = opts.allProperties[k];
		if (prop.unique === true) {
			definitions.push("UNIQUE (" + k + ")");
		} else if (prop.index) {
			definitions.push("INDEX (" + k + ")");
		}
	}
	...
	for (k in opts.allProperties) {
		prop = opts.allProperties[k];
		if (prop.unique === true) {
			definitions.push("UNIQUE KEY " + k + " (" + k + ")");
		} else if (prop.index) {
			definitions.push("INDEX (" + k + ")");
		}
	}
	...
	queries.push(
		"CREATE TABLE " + driver.config.schema + '.' + opts.table +
		" (" + definitions.join(", ") + ")"
	);
	...
	for (i = 0; i < opts.many_associations.length; i++) {
		...
		queries.push(
			"CREATE TABLE " + driver.config.schema + '.' + opts.many_associations[i].mergeTable +
			" (" + definitions.join(", ") + ")"
		);
	}
